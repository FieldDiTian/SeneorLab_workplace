...
class MotorController:
    def __init__(self, port='COM8', baudrate=115200, auto_enable=True, default_feedrate=1000, wait_time=0.5):
        """
    ...
    def wait_for_movement(self):
        """等待所有移动完成 / Wait for all movements to complete"""
        # 根据距离和速度估算移动时间，然后简单等待
        # 由于Marlin的M400和位置查询在某些配置下不可靠，使用基于时间的等待
        print("    等待移动完成...")
    ...
    def move_axis(self, axis, steps, feedrate=None, wait=True):
        """
        移动指定轴指定步数 / Move specified axis by specified steps
        
        参数 / Parameters:
        - axis: 轴名称 / Axis name ('X', 'Y', 'Z', 'I', 'J', 'K', 'U', 'V', 'W', 'E')
        - steps: 移动步数 / Movement steps (可以是正数或负数)
        - feedrate: 进给速度 (步/分钟)，如果为None则使用默认值 / Feed rate (steps/min), use default if None
        ...
        # 添加30%的安全余量
        safe_wait_time = estimated_time * 1.3 + 2  # 至少等待2秒
        
        # 执行移动命令 / Execute movement command
        cmd = f"G1 {axis}{steps} F{feedrate}"
        print(f"\n移动 / Moving {axis}轴 / axis {steps}步 (速度 / speed: {feedrate}步/分钟)...")
        print(f"    预估时间: {estimated_time:.1f}秒, 等待时间: {safe_wait_time:.1f}秒")
        self.send_gcode(cmd)
        
        # 等待移动完成 / Wait for movement to complete
        if wait:
            # 先等待预估的移动时间
            print(f"    等待 {safe_wait_time:.1f} 秒...")
    ...
    def move_x(self, steps, feedrate=None, wait=True):
        """移动X轴指定步数 / Move X-axis by specified steps"""
        self.move_axis('X', steps, feedrate, wait)
    
    def move_y(self, steps, feedrate=None, wait=True):
        """移动Y轴指定步数 / Move Y-axis by specified steps"""
        self.move_axis('Y', steps, feedrate, wait)
    
    def move_z(self, steps, feedrate=None, wait=True):
        """移动Z轴指定步数 / Move Z-axis by specified steps"""
        self.move_axis('Z', steps, feedrate, wait)
    
    def move_i(self, steps, feedrate=None, wait=True):
        """移动I轴指定步数 / Move I-axis by specified steps"""
        self.move_axis('I', steps, feedrate, wait)
    
    def move_j(self, steps, feedrate=None, wait=True):
        """移动J轴指定步数 / Move J-axis by specified steps"""
        self.move_axis('J', steps, feedrate, wait)
    
    def move_k(self, steps, feedrate=None, wait=True):
        """移动K轴指定步数 / Move K-axis by specified steps"""
        self.move_axis('K', steps, feedrate, wait)
    
    def move_u(self, steps, feedrate=None, wait=True):
        """移动U轴指定步数 / Move U-axis by specified steps"""
        self.move_axis('U', steps, feedrate, wait)
    
    def move_v(self, steps, feedrate=None, wait=True):
        """移动V轴指定步数 / Move V-axis by specified steps"""
        self.move_axis('V', steps, feedrate, wait)
    
    def move_w(self, steps, feedrate=None, wait=True):
        """移动W轴指定步数 / Move W-axis by specified steps"""
        self.move_axis('W', steps, feedrate, wait)
    
    def move_e(self, steps, feedrate=None, wait=True):
        """移动E轴(挤出机)指定步数 / Move E-axis (extruder) by specified steps"""
        self.move_axis('E', steps, feedrate, wait)
